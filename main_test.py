...
def main():
    ...
    if 'cifar' in args.dataset.lower():
        train_dataset, val_dataset, znorm = data_loaders.cifar10()
    elif args.dataset.lower() == 'svhn':
    ...
    if len(args.bbmodel) > 0:
        ...
        bbstate_dict = torch.load(os.path.join(model_dir, args.bbmodel+'.pth'), map_location=torch.device('cpu'),weights_only=False)
...
#         logger.info(json.dumps(atk_config))
        ...
        state_dict = torch.load(os.path.join(model_dir, args.identifier + '.pth'), map_location=torch.device('cpu'),weights_only=False)
